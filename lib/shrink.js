...
const mutate = function (coll) {
	...
	const mutator  = [mutate, substitute][Math.floor(Math.random() * 2)]
...
module.exports = function (tester, prop, tcase) {

	const bigNumber = 1000000000000000

	GA(tcase, mutate, function (tcase) {
		...
		return stillFails && stillValid? score(tcase): bigNumber
	})
}
